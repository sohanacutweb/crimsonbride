...
class Photonic_Flow_Manager {
	...
	function __construct() {
		...
		$this->shortcode_attributes = array(
			'common' => array(
				'columns', 'count', 'more', 'photo_count', 'photo_more', 'title_position', 'caption', 'media', 'main_size', 'thumb_size', 'tile_size', 'video_size', 'popup', 'thumbnail_effect', 'show_gallery', // All lightbox
		...
		$this->check_for_blanks = array(
			'more', 'photo_more', 'count', 'photo_count',
		);
	}
	...
	private function construct_shortcode() {
		...
		if (!empty($input)) {
			...
			foreach ($attr_array as $key => $value) {
				if (in_array($key, array('more', 'photo_more', 'count', 'photo_count', 'show_gallery')) && empty($short_code[$key])) {
					unset($attr_array[$key]);
				}
			}
